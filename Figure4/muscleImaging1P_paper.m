...
% Define optogenetic stimulus start frames based on protocol for
% trigerring. each stimulus lasts 1.6s
maxLastFrame = 2500;
trialStartFrames = 10*fps+1:11.6*fps:maxLastFrame;
...
for i = 1:length(realSubFolders)
    ...
    genotypeSubFolders = genotypeSubFolders (3:end);% removes directory folders and leaves real subfolders.
    
    flexedsubFolders = genotypeSubFolders(startsWith({genotypeSubFolders.name},{'flex'})); % folders contained flexed Fe-Ti trials
    extendedsubFolders = genotypeSubFolders(startsWith({genotypeSubFolders.name},{'extend'}));% folders containing extended Fe-Ti trials
    ...
    for j=1:data(i).numFlexedFlies
        ...
        for k = 1:data(i).perFlyData(j).numFlxTrials
            ...
            data(i).perFlyData(j).perFlxTrialData(k).FlxF = data(i).perFlyData(j).perFlxTrialData(k).FlxF(:,1:end-1);%remove background column 
